...
describe('SessionListView', () => {
  ...
  describe('tag filter', () => {
    ...
    it('shows empty state when no sessions match the combined filters', () => {
      ...
      window.history.replaceState(
        null,
        '',
        '/?tag=architecture&stage=Yellow+Stage',
      )
      renderView()
  ...
  describe('bookmark buttons in session cards', () => {
    ...
    describe('hide past events toggle', () => {
      ...
      it('renders the "Hide past events" checkbox', () => {
        renderView()
        expect(
          screen.getByRole('checkbox', { name: /hide past events/i }),
        ).toBeInTheDocument()
      })
  ...
  describe('timeslot separators', () => {
    ...
    it('displays the start time in each separator', () => {
      renderViewWithResult()
      expect(
        screen.getByRole('separator', { name: '09:30' }),
      ).toBeInTheDocument()
      expect(
        screen.getByRole('separator', { name: '10:00' }),
      ).toBeInTheDocument()
      expect(
        screen.getByRole('separator', { name: '10:30' }),
      ).toBeInTheDocument()
    })
